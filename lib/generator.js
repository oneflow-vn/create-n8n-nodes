...
const createFileContext = (data, tag = null, operations = [], operation = null) => {
...
codegen.generate = async (config) => {
  try {
    ...
    config.nodeName = titleCase(_.camelCase(config.openapi.info.title));
    ...
    // update package.json
    config.package.name = `n8n-nodes-${_.kebabCase(config.openapi.info.title)}`;
