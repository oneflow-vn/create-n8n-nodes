...
const generateOperationFiles = ({
  ...
  const paths = _.get(data.openapi, 'paths', []);

  const createFiles = Object.keys(paths).map(key => {
    const operationsMap = paths[key];
    const methods = Object.keys(operationsMap);

    return methods.map(method => {
      const operation = operationsMap[method];
  ...
  Promise.all(createFiles.flat()).then(resolve).catch(reject);
});
...
const generateDirectoryStructure = config => new Promise((resolve, reject) => {
  ...
  walker.on('directory', async (root, stats, next) => {
    ...
    try {
      ...
      } if (stats.name.includes('[$nodeName]')) {
        const nodeName = _.camelCase(config.openapi.info.title);
        const newFolderName = stats.name.replace('[$nodeName]', nodeName);
        const subdir = root
          .replace(new RegExp(`${templates_dir}[/]?`),'');
        const targetPath = path.resolve(target_dir, subdir, newFolderName);
        xfs.mkdirpSync(targetPath);
      }  else {
