  ...
  new Promise((resolve, reject) => {
    try {
      if (config.deleteFolders) {
        console.warn(
          yellow(
            `Deleting all subfolders named '${config.deleteFolders}' in ouput directory '${config.targetDir}'`
          )
        );

        for (const folder of config.deleteFolders) {
          const folderPath = folder.replace('[$nodeName]', config.nodeName);
          const folderPathRendered = path.resolve(config.targetDir, folderPath);
          console.log('Deleting:', folderPathRendered);
  ...
  new Promise((resolve, reject) => {
    ...
    walker.on('file', async (root, stats, next) => {
      try {
        ...
        const targetFilePath = path.resolve(config.targetDir, srcFileSubPath);
...
codegen.generate = async (config) => {
  try {
    ...
    // TODO: validate the OpenAPI file
    // try {
    //   await SwaggerParser.validate(bundledJSON, { });
    // } catch (e) {
    ...
    _.defaultsDeep(config, {
      ...
      targetDir: path.resolve(os.tmpdir(), 'openapi-generated'),
