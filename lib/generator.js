...
const generateResourceFolder = async (config, tag, operation) => {
  ...
  const subdir = path.resolve(config.target_dir, path.relative(config.templates_dir, config.root))
...
const copyNodeIcons = config => new Promise((resolve) => {
  ...
  if (fs.existsSync(iconPath)) {
    ...
    const nodePath = path.resolve(config.target_dir, 'nodes', config.nodeName);
