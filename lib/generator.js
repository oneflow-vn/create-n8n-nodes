...
const generateFile = options => new Promise((resolve, reject) => {
  ...
  const rendered_path = target_path.replace('[$nodeName]', _.camelCase(data.openapi.info.title));
...
const getOperationContext = (data, operation) => {
  const url = operation.path.replace(/{/g, '${');
  const httpOptions = {
...
const createFileContext = (data, tag, operations, operation) => {
...
const flattenOperations = (schema) => {
  return _.flatMap(schema.paths, (pathObj, pathName) => {
    return _.map(pathObj, (operation, method) => {
      return _.merge({}, operation, {
        path: pathName,
        method,
      });
    });
...
const flattenOperationsByTag = (schema, tag) => {
  const allOperations = flattenOperations(schema);
  return allOperations.filter(operation => {
    return operation.tags && operation.tags.includes(tag);
  });
};
...
const generateOperationFiles = ({
  ...
  const operations = flattenOperationsByTag(data.openapi, tag.name);
  ...
  const createFiles =  operations.map(operation => {
    ...
    const target_path = path.resolve(target_dir, template_path)
      ...
      .replace('[$operation]', _.kebabCase(operation.operationId));

    const context = createFileContext(data, tag, operations, operation);
    return generateFile({
      templates_dir,
      target_dir,
      file_name,
...
const generateResourcesFile = ({
  ...
  const createFiles = tags.map(tag => {
    ...
    const template_path = path.relative(templates_dir, path.resolve(root, file_name));
    const target_path = path.resolve(target_dir, template_path).replace('[$resource]', _.kebabCase(tag.name));

    const operations = flattenOperationsByTag(data.openapi, tag.name);
    const context = createFileContext(data, tag, operations, null);
 ...
 * @returns {Promise}
 */
const generateResourceFolder = async (config, tag, operationName) => {
  ...
  const subdir = path.resolve(config.target_dir, path.relative(config.templates_dir, path.resolve(config.root, config.file_name)))
    .replace('[$nodeName]', _.camelCase(config.data.openapi.info.title))
    .replace('[$resource]', _.kebabCase(tag.name));

  let targetPath = path.resolve(config.target_dir, subdir, newFolderName);

  if (operationName) {
    targetPath = targetPath.replace('[$operation]', _.kebabCase(operationName));
  }
 ...
 */
const generateOperationFolders = async (config, tag) => {
  console.log('Generating operation folders:', config.root, config.file_name, tag.name);
  const paths = _.get(config.data.openapi, 'paths', []);

  for (const method in paths) {
    const operations = paths[method];
    for (const operation_name in operations) {
      ...
      await generateResourceFolder(config, tag, operation.operationId);
    }
  }
};
...
const generateResourceFolders = async (config) => {
  ...
  for (const tag of tags) {
    if (config.file_name.includes('[$operation]') || config.root.includes('[$operation]')) {
      await generateOperationFolders(config, tag);
    } else {
      await generateResourceFolder(config, tag);
    }
  }
};
...
const generateDirectoryStructure = config => new Promise((resolve, reject) => {
  ...
  walker.on('directory', async (root, stats, next) => {
    ...
    const dirPath = dirPathTpl.replace('[$nodeName]', _.camelCase(config.openapi.info.title));
